...
Blockchain.prototype.createNewTransaction = (amount, sender, recipient)=>{ //트랜잭션 생성함수
	const newTransaction = {
		amount: amount,  //규모(금액)
		sender: sender,  //발신자
		recipient: recipient, //수신자
		transactionId: uuid().split('-').join(""), //구분값
	}
	return newTransaction;
}
Blockchain.prototype.addTransactionToPendingTransactions = (transactionObj)=>{ //미결 트랜잭션
	this.pendingTransactions.push(transactionObj);
	return this.getLastBlock()['index']+1;
}

Blockchain.prototype.hashBlock = (previousBlockHash, currentBlockData, nonce)=>{ // 해시함수
	const dataAsString = previousBlockHash + nonce.toString() + JSON.stringify(currentBlockData);
	const hash = sha256(dataAsString); //해싱(256bytes  암호화)
...
Blockchain.prototype.getTransaction = (transactionId)=>{ //transactionId로 조회하는 함수
	...
	this.chain.forEach(block =>{ // 전체블럭 색인
		block.transactions.forEach(transaction =>{  //색인
...
Blockchain.prototype.getAddressData = (address)=>{ // 주소값을 통한 내용 색인 함수
	const addressTransactions = [];
	this.chain.forEach(block =>{ // 색인
	...
	let balance = 0; 
	addressTransactions.forEach(transaction =>{  //잔고 색인
